kill_imp_spell() {
  local p=$(COLUMNS=512 ps -cA | grep "$(gettext "mischievous_imp")" | awk '{print $1}')
  # argh, ps -Ao ... doesn't find any process in macos-latest
  # and   ps -cAo ... doesn't find any process in ubuntu-latest
  # let's try both!
  COLUMNS=512 ps -cAo pid,comm,ppid 2>/dev/null | grep "$p$" | grep "$(gettext "spell")" | awk '{print $1}' | xargs kill -9 2>/dev/null
  COLUMNS=512 ps -Ao pid,comm,ppid | grep "$p$" | grep "$(gettext "spell")" | awk '{print $1}' | xargs kill -9 2>/dev/null
}
cellar=$(eval_gettext '$GSH_HOME/Castle/Cellar')


kill_imp_spell
rm -f "$cellar"/*_"$(gettext "coal")"
gsh check

unset -f kill_imp_spell
unset cellar

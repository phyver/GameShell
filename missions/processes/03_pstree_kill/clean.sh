...
ps -e | awk -v imp="$(gettext "mischievous_imp")" '$0 ~ imp {print $1}' | xargs kill -9 2> /dev/null
ps -e | awk -v fairy="$(gettext "nice_fairy")" '$0 ~ fairy {print $1}' | xargs kill -9 2> /dev/null
ps | awk '$0 ~ /sleep|tail/ {print $1}' | xargs kill -9 2> /dev/null

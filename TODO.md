...
--------
  ...
  - [ ] rething end of game (when no more mission is found)
...
---------
  ...
  - [ ] apparently, the only reason we require `coreutils` on macos is to get
        `greadlink`. It might be worth using a POSIX `realpath` function to
        remove the dependency. (We might have to change `checksum` to use
        macos' native sha1 checksum.)
